...
unsigned long motor2_time = millis();
int motor2_update_i = 0;
...
long motor1_tick_start = 0;
unsigned long motor1_time_start = 0;

float get_motor1_speed(){
  if (motor1_update_i == 0) {
    motor1_tick_start = get_motor1_ticks();
    motor1_time_start = millis();
    motor1_update_i++; 
  }
  else if (millis()-motor1_time_start >= measure_interval){//motor1_update_i < update_interval) {
    long delD = get_motor1_ticks() - motor1_tick_start;
    unsigned delT = millis() - motor1_time_start;
    motor1_speed = float(delD) / float(delT);
    motor1_update_i = 0;
  }
  return motor1_speed;
}
...
long motor2_tick_start = 0;
unsigned long motor2_time_start = 0;

float get_motor2_speed() {
  if (motor2_update_i == 0) {
    motor2_tick_start = get_motor2_ticks();
    motor2_time_start = millis();
    motor2_update_i++;
  }
  else if (millis()-motor2_time_start >= measure_interval){//motor2_update_i < update_interval) {
    long delD = get_motor2_ticks() - motor2_tick_start;
    unsigned delT = millis() - motor2_time_start;
    motor2_speed = float(delD) / float(delT);
    motor2_update_i = 0;
  }
  return motor2_speed;
}
